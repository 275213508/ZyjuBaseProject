    ...
    public static Bitmap createBitmapFromView(View view, float scale) {
        ...
        if (view instanceof NestedScrollView) {
            for (int i = 0; i < ((NestedScrollView)view).getChildCount(); i++) {
                h += ((NestedScrollView)view).getChildAt(i).getHeight();
//                ((NestedScrollView)view).getChildAt(i).setBackgroundColor(Color.WHITE);//图片背景色,
            }
            bitmap = createBitmapSafely((int) (view.getWidth() * scale),
